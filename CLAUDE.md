...
- `src/core/**` — pure logic, **no `vscode` imports**: parsers (`linkParser`, `embedParser`, `refRange`), `fenceMask`, `frontmatter`, extractors (`headingExtractor`, `blockIdExtractor`, `sectionSlice`), `resolver/resolveTarget`, `rename/rewriteWikiRefs`, `completion/rankCompletions`, `completion/rankFragmentCompletions` (headings + block IDs after `#`), `diagnostics/findBrokenRefs` (the one definition of a broken link, shared by live diagnostics and the workspace scan), `imageSize` (intrinsic dimensions from image headers), `pathFilter` (excluded-folder matching), `textPosition` (offset → line/character without a TextDocument), `footnote` (strips `[^ref]` markers; one scanner each for reference tokens and for definitions with their continuation lines, behind `extractFootnoteDefs`, `footnoteRefAt`, `footnoteDefAt`, `footnoteRefsFor` — only the `[^ref]` token of a definition counts as the definition, so links in a footnote's body stay with their own providers). Unit-tested in plain Node.
- `src/adapters/**` — VSCode glue: `indexService`, `workspaceBoundary`, the providers (`documentLinkProvider`, `hoverProvider`, `completionProvider`), `renameHandler`, `footnoteHoverProvider` (hover on `[^ref]` shows the same-file definition), `footnoteDefinitionProvider` (F12 from a `[^ref]` to its definition and from the definition's `[^ref]` token back to its references), `footnoteReferenceProvider` (Shift+F12: every reference plus the definition), `ranges` (offset interval → `vscode.Range`, the one conversion providers share), `diagnostics` (live per-open-document diagnostics plus the on-demand `wikiLinks.scanWorkspace` command, which reads unopened files from disk via `workspace.fs` as UTF-8, builds ranges with `core/textPosition`, and keeps scanned results until the next scan or the file is deleted/renamed — VSCode file events plus a delete-only `FileSystemWatcher` for external deletes), `decorations` (resolution-based editor coloring), `previewResolver`. May import `vscode` and `src/core/**`.
- `src/markdownItPlugin/**` — the wiki plugin contributed to the Markdown preview via `contributes["markdown.markdownItPlugins"]`. Rewrites `[[...]]` into navigable links (an unresolved link is left as its original `[[...]]` text, escaped, so the reader can see the broken reference) and expands `![[...]]` embeds. VSCode reuses the contributed markdown-it instance for source-editor analyses too (document highlights, smart-select, symbol mapping), so the rule **gates on `state.env` having a preview marker** (`containingImages` / `currentDocument` / `resourceProvider`) before touching `state.src` — otherwise rewritten ranges leak back to the source editor as bogus decorations. VSCode tokenizes the preview by string, so a core rule cannot read the previewed file path directly; the extension supplies it via `getDocumentPath` (backed by `vscode.window.activeTextEditor`) to seed the embed-cycle guard. **No `vscode` imports** (runs in the preview process).
- `src/previewScript/**` — code injected into the Markdown preview **webview** via `contributes["markdown.previewScripts"]` (with `media/footnoteHover.css` via `previewStyles`), bundled by esbuild as a browser IIFE into `dist/previewScript/`. `footnoteHover.ts` shows a hover card for a footnote reference by cloning the `<li id="fn…">` body a footnote plugin (e.g. `bierner.markdown-footnotes`) already rendered — inert when no such plugin is present. Listeners are delegated on `document` because the preview replaces its DOM on every update. Browser APIs and `src/core` only — **no `vscode`, no Node modules, no markdown-it** (ESLint-enforced); pure decisions live in exported helpers with unit tests, DOM wiring stays thin.
...
- **Heading fragments match by exact text or by slug; paired brackets are plain text.** A fragment or display segment accepts one level of paired `[..]` as text (all four ref regexes — both core parsers and both in `wikiRule` — stay in sync), so `[[note#Edge cases [brackets]]]` parses. `|` still starts display text, and unpaired/nested brackets cannot be written, so `rankFragmentCompletions` inserts such headings as their slug (label stays the text); slug-written links resolve through the same `findHeading` match.
- **Footnote references are stripped from heading text.** `Heading.text`/`slug` are footnote-free (`core/footnote.ts`), so `## Setup[^1]` is matched, hover-sliced, and offered in completion as `Setup`. **Preview anchors are computed, not guessed.** VSCode assigns heading ids by slugging the concatenated text of the heading's inline tokens (GitHub slugifier, `-1`/`-2` suffixes for duplicates), and which tokens exist depends on the plugins loaded into the shared markdown-it instance — with `bierner.markdown-footnotes` (markdown-it-footnote) `[^1]` is a text-less `footnote_ref`, so `## Setup[^1]` is `#setup`; without it, `#setup1`. `markdownItPlugin/headingAnchors.ts` therefore tokenizes the target with the live instance (`previewHeadingAnchors` → line → id); the plugin uses it directly for same-file links (over the expanded preview text) and hands it to `WikiResolver.resolveLink` as `anchorsOf` so `previewResolver` can apply it to a cross-file target's text, which it reads with an mtime-keyed bounded cache. Unit-tested against the real `markdown-it-footnote`.
